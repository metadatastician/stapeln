...
[IMPORTANT]
====
**This is an inherited copy of the generic RSR *template* outline. It describes the RSR
template repository, not stapeln.** Where it disagrees with this project, this project wins.

Specifically: it lists `LICENSE.txt` as "AGPL + Palimpsest dual license". **stapeln is
MPL-2.0 for code and CC-BY-SA-4.0 for documentation**, and the file is `LICENSE`, not
`LICENSE.txt`. See link:LICENSING.adoc[LICENSING.adoc]. It also says "README.adoc | This file",
which is true of the template, not of this document.

Retained as a reference for RSR structure. See link:DEBT.adoc[DEBT.adoc] D-7.
====
...
|`LICENSE.txt`
|AGPL + Palimpsest dual license _(template default — stapeln uses `LICENSE`, MPL-2.0)_
...
* `README.adoc`
* `RSR_COMPLIANCE.adoc`
* `LICENSE.txt` (AGPL + Palimpsest) _(template default — stapeln uses `LICENSE`, MPL-2.0)_
