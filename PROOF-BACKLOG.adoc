...
* *Alire 2.0.2 {plus} GNAT 14.2.1 {plus} gnatprove 14.1.1* (for SPARK), via `alr` (`/usr/local/bin/alr`).
...
[[white_check_mark-verified-clean-compile-green]]
...
|selur `idris/++{++Proofs,Theorems}.idr` (was `container-stack/selur/…`) |`--check` clean
...
[[red_circle-remaining-proof-work-drive-to-clear--user-bar-discharge-everything]]
== 🔴 REMAINING PROOF WORK (drive to "clear" — user bar: DISCHARGE EVERYTHING)

[[1-abi-layer-repair--white_check_mark-done-srcabiabiipkg-builds-exit-0-honest]]
...
relocate the files into `Stapeln/ABI/` {plus} set `sourcedir` accordingly.
...
Zig source of truth ({plus} `Result` alias); proofs now describe the actual boundary.
...
* `Foreign.idr`: `Handle`/`createHandle` were undefined — added (Bits64 wrapper {plus}
...
[[2-container-stackvordrsrcabi--white_check_mark-done-orphan-repaired-builds]]
=== 2. vordr `src/abi/` — ✅ DONE (orphan repaired, builds)
...
layer {plus} `resultToInt`/`DecEq` {plus} injectivity & round-trip proofs,
...
[[3-cerro-torre-normalizedissafe--white_check_mark-discharged-2026-06]]
...
path. Redesigned SafePath (SafeSingle leaf {plus} '/'-free via `charsElem` {plus} non-empty
...
`splitNoDelim` (split components are '/'-free) {plus} `dotDotInfixOfJoin` (a ".." component
...
`alr -n build ++||++ true && alr -n exec -- gnatprove -P cerro_torre.gpr --level=2 --report=fail --output-msg-only`,
run inside a clone of `metadatastician/cerro-torre`.

[[5-ats-dats-6-files--never-verified-no-ats2-toolchain]]
=== 5. ATS `.dats` (6 files) — never verified (no ATS2 toolchain)

`cerro-torre/verification/ats/++*++.dats` {plus} `tools/ats-shadow/++*++.dats`. Needs `postiats`
...
=== 6. Final: PROOF-NEEDS {plus} trusted-base audit estate-wide; confirm zero
...
cerro-torre: 8 crypto AXIOM-STUBs (hardness/composition) {plus} 4 string-primitive
bridges (`isPrefixOfBridge`, `unpackAppend`, `eqStringSym`, `unpackEmptyInv`) {plus}
*2 path-normalization axioms* (`splitNoDelim`, `dotDotInfixOfJoin`, from the
normalizedIsSafe discharge) {plus} 4 CryptoFFI runtime stubs. Zero believe_me/cast
Refl/assert_total in cerro-torre or vordr proof code.

[[green_circle-false-theorems-found--fixed-this-session-do-not-regress]]
...
* vordr `nonAdminCantPrivilege` (unsound {plus} undefined types) → disabled w/ rationale.

[[package-side-projects--white_check_mark-handed-off-filed-as-github-issues-2026-06-05]]
== 📦 SIDE-PROJECTS — ✅ HANDED OFF (filed as GitHub issues 2026-06-05)

[arabic]
. *AffineScript migration* → *stapeln++#++92*. `.ts`(9, banned){plus}`.res`(164)→AffineScript.
...
. *CI hardening remainder (Hypatia highs/criticals)* → *stapeln++#++91*. `instant-sync.yml`
...
. *svalinn greenfield proofs* → *svalinn++#++34*. 177 `Obj.magic` in auth/MCP/policy;
gated on the AffineScript migration. svalinn PR ++#++32 also red on pre-existing Type/Lint/Format.
. *Estate CLAUDE.md sweep* → *stapeln++#++93*. propagate affinescript→AffineScript
reconciliation to the other repos via the rsr-template-repo (only stapeln{plus}svalinn done here).

The Hypatia advisory scan (444 findings: 20 critical / 149 high / 275 medium) is
`--exit-zero` (non-blocking); its findings == items 1{plus}2 above. The code-scanning
...
[[done--shipped-pr-hyperpolymathstapeln89-all-gates-green]]
== Done & shipped (PR hyperpolymath/stapeln++#++89, all gates green)

cerro-torre discharges (ociLayoutEnforcement, absolutePathRejection {plus} SafePath fix);
vordr repair; SPARK gate fix (config-gen {plus} SPARK_Mode consistency); CI root-fixes
(A2ML carve-out, `.hypatia-baseline.json` for .res/.dats, workflow pin{plus}timeouts);
CLAUDE.md affinescript→AffineScript reconciliation (stapeln ++#++89 {plus} svalinn ++#++32).
