...
|*No provenance* |Can't explain _why_ |"Trust me, it's secure" (NOT ACCEPTABLE)
|===
...
|*Transparent* |Shows _why_ it flagged something |Audit trail for compliance
|===
...
[arabic]
. *User configures stack* in UI (affinescript)
. *Stack model sent* to reasoning engine (Scheme {plus} miniKanren)
. *Rules applied* to find violations
. *Results returned* with provenance (why it's a problem)
. *UI shows issues* with fix suggestions
. *Rules updated* daily from CVE/OWASP feeds
...
== Optional: LLM for Natural Language _After_ miniKanren
...
*Key*: LLM explains _after_ miniKanren decides. Logic first, language second.
...
* [ ] Set up Guile Scheme {plus} miniKanren
...
[arabic]
. *Deterministic* = Trustworthy for security
. *Provable* = Full audit trail for compliance
. *Updateable* = Add new CVEs immediately
. *Transparent* = Users understand _why_
. *Fast* = Milliseconds, not seconds
. *Small* = No GPU, no 1GB models

*LLM role*: Optional, for natural language explanations _after_ miniKanren decides

*Result*: stapeln becomes the _most trustworthy_ container security tool because every decision is logically provable and traceable to OWASP/NIST/CVE sources.
