...
* [ ] Create semantic HTML {plus} XML metadata
...
[[42-ephapax-linear-types-or-rust-mvp]]
...
|*Backend* |Elixir {plus} Phoenix |API server, WebSocket
...
* *M3 (Week 5)*: Backend API {plus} GraphQL
...
* [ ] Support for 100{plus} component stacks
...
|WebSocket scaling issues |Use Phoenix Presence {plus} PubSub
...
[arabic]
. *Immediate*: Set up affinescript {plus} Deno development environment
. *This week*: Complete Phase 1 (Frontend foundation)
. *Next week*: Implement drag-and-drop (Phase 2)
. *Two weeks*: Backend setup (Phase 3)
