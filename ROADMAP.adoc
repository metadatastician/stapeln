...
|Undo/redo system |Working |Snapshot-based, 50-depth cap, Ctrl{plus}Z/Ctrl{plus}Y
|Auto-save |Working |30-second interval with dirty tracking
|Dark mode |Working |System detection {plus} localStorage persistence
|Import/Export |Working |File picker, JSON {plus} compose files, real container images
|Security UX (4 views) |Working |Port config (1165 LOC), security inspector (832), gap analysis (951), simulation (1622)
|Backend (Phoenix REST {plus} GraphQL) |Working |CRUD {plus} validation via NativeBridge
|Zig FFI shared library |Working |Real CRUD {plus} validate {plus} dispatch, SHA-256 {plus} Ed25519
...
* [x] OWASP ModSecurity {plus} firewall configuration spec
...
* [x] Dark mode (system detection {plus} localStorage persistence)
...
* [x] Zig FFI shared library (real CRUD {plus} validate {plus} dispatch)
* [x] Idris2 ABI with 8 genuine proofs
* [x] Real SHA-256 {plus} Ed25519 in crypto.zig
...
* [x] JWT {plus} Plug module present
...
* [ ] Guile Scheme {plus} miniKanren library setup
* [ ] 10{plus} security rules (SSH exposure, root user, unencrypted traffic, etc.)
...
[[v12-enterprise-features-q1-2027]]
...
[[v20-kubernetes-integration-2027]]
...
*Status*: Phase 2 complete, Phase 3 ~75% (backend core {plus} simulation engine done, auth/VeriSimDB primary/miniKanren/firewall pending)
