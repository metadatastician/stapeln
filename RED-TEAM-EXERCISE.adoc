...
|===
|Attack |Time |Success? |Reason
|Default creds |2s |❌ Blocked |PAM auth {plus} 2FA
|SQL injection |2s |❌ Blocked |Parameterized queries
|Network scan |2s |❌ Blocked |firewalld
|Ephemeral pinhole |2s |❌ Blocked |30s max, IP restricted
|Command injection |30s |❌ Blocked |Fuzz tested {plus} WAF
|Container breakout |1m |❌ Blocked |Cape detects {plus} kills
|Secret exfiltration |1m |❌ Blocked |Fjord (encrypted)
|MITM |2m |❌ Blocked |Strait (mTLS)
|Supply chain |30m |❌ Blocked |Binary verification
|TOCTOU |1h |❌ Blocked |Re-validation {plus} signing
|===
...
[arabic]
. 🔴 *Build Fjord* (secrets manager) - Without this, instant pwn via env vars
. 🟠 *Reduce ephemeral pinhole default to 30s* - 5 minutes is too long
. 🟠 *Add source IP restrictions to pinholes* - Only allow specific IPs
. 🟠 *Prevent privileged mode by default* - Require explicit Settings enable {plus} confirmation
. 🟠 *Re-validate before deploy* - TOCTOU protection
. 🟠 *Fuzz test all inputs* - Command injection protection
. 🟠 *Verify Cerro Torre binary signature* - Supply chain protection
. 🟡 *Add 2FA support* - Extra auth layer
. 🟡 *Build Cape* - Runtime threat detection
. 🟡 *Build Strait* - mTLS between services
...
*Current State*: He probably CAN'T destroy it in 2 seconds (firewalld {plus} PAM prevent quick wins), but he COULD destroy it in 2 minutes (secret exfiltration via env vars).

*Target State*: After building Fjord {plus} Cape {plus} Strait, he shouldn't be able to destroy it even in 2 hours.
