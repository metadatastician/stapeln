...
// broken `just` recipe. Each is listed in docs/BADGES.md with exactly what is
...
Licensing: **MPL-2.0** for code, **CC-BY-SA-4.0** for documentation — see link:LICENSING.md[LICENSING.md].
Badges wanted and pending restoration: link:docs/BADGES.md[docs/BADGES.md].
...
link:STATUS.md[**`STATUS.md`** is the canonical source of truth] — it records only claims
backed by a file read, a command run, or a CI query, and it names the command so you can
reproduce the number. link:DEBT.md[**`DEBT.md`**] indexes known debt across licence,
