...
image:https://img.shields.io/badge/frontend-ReScript%20%2B%20React-e34c4c.svg[ReScript]
image:https://img.shields.io/badge/backend-Elixir%20%2B%20Phoenix-4e2a8e.svg[Elixir]
image:https://img.shields.io/badge/proofs-Idris2-6b4fbb.svg[Idris2]
image:https://api.scorecard.dev/projects/github.com/metadatastician/stapeln/badge[OpenSSF Scorecard,link="https://scorecard.dev/viewer/?uri=github.com/metadatastician/stapeln"]

// Six badges are WANTED and awaiting restoration — WCAG 2.3 AAA, OWASP, SOC 3,
// ISO 27001, CIAQ and Green Hosting. They were removed on 2026-08-07 only because
// nothing in this repository verified them. OpenSSF Best Practices was removed
// separately on 2026-09-22 because project 8509 belongs to PA-updated-Weapon-Master.
// A CRG grade badge is blocked by a broken `just` recipe. Each is listed in
// docs/BADGES.adoc with exactly what is needed to put it back; they are not abandoned.

Licensing: **MPL-2.0** for code, **CC-BY-SA-4.0** for documentation — see link:LICENSING.adoc[LICENSING.adoc].
Badges wanted and pending restoration: link:docs/BADGES.adoc[docs/BADGES.adoc].
...
link:STATUS.adoc[**`STATUS.adoc`** is the canonical source of truth] — it records only claims
backed by a file read, a command run, or a CI query, and it names the command so you can
reproduce the number. link:DEBT.adoc[**`DEBT.adoc`**] indexes known debt across licence,
documentation, code, proof, test, CI/CD and supply chain.

A per-component table used to live here. It drifted six months out of date and was wrong in
both directions — understating the Idris2 proofs (called "stubs"; there are 8 genuine
modules with zero escape hatches), miniKanren, VeriSimDB and authentication (all called "not
started"; all substantial), while overstating compliance. Rather than maintain two tables
that disagree, this one is gone. Highlights only:

* **Works:** 41 REST routes + Absinthe GraphQL; 10 UI tabs (TEA); full-fidelity design
  persistence and working security-scan / gap-analysis (both fixed in
  https://github.com/metadatastician/stapeln/pull/17[#17]); 8 honest Idris2 ABI proofs; real
  Ed25519/SHA-256 in Zig; six code-generation emitters
* **Missing:** no `mix test` gate on GitHub; `scripts/readiness-check.sh` invoked by nothing;
  no persistent database (in-memory GenServer only)
...
See link:ROADMAP.adoc[ROADMAP.adoc] for detailed timeline.
...
* link:UX-MANIFESTO.adoc[UX Manifesto] - Design philosophy
* link:CONTAINER-HATER-TEST.adoc[Container-Hater Test] - Ultimate UX challenge
* link:SECURITY-STACK-AUDIT.adoc[Security Audit] - Current compliance (47% → 100%)
* link:FIREWALL-CONFIG.adoc[Firewall Config] - OWASP ModSecurity + ephemeral pinholes
* link:SECURITY-REASONING-ENGINE.adoc[Security Engine] - miniKanren vs SLM comparison
* link:DATABASE-AND-DOCS-INTEGRATION.adoc[Database Integration] - VeriSimDB + A2ML + K9-SVC
* link:UI-MOCKUPS.adoc[UI Mockups] - Game-like interface designs
* link:SETUP.adoc[Setup Guide] - Deno-only installation
