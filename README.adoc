...
// Six further badges are WANTED and awaiting restoration — WCAG 2.3 AAA, OWASP,
// SOC 3, ISO 27001, CIAQ and Green Hosting — plus a CRG grade badge blocked by a
...
* **Missing:** no `mix test` gate on GitHub; `scripts/readiness-check.sh` invoked by nothing;
  no persistent database (in-memory GenServer only); five uninitialised `container-stack/`
  submodules
