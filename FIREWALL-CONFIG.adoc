...
[arabic]
. *OWASP ModSecurity* - Web application firewall at gateway (Svalinn)
. *Default-deny firewall* - Whitelist-only approach (firewalld/nftables)
. *Ephemeral pinholes* - Temporary port openings with auto-expiry
. *User-only access* - Strong authentication for stapeln tool itself
. *Visual port control* - Simple UI, no CLI commands needed
...
[[2-firewalldnftables-default-deny-rules]]
...
[arabic]
. Click component
. Port section → Port 8080 → Select "Ephemeral"
. Choose duration: 5 minutes
. Done ✅
...
[arabic]
. *Implement Elixir backend* with EphemeralPinhole GenServer
. *Add ModSecurity* to Svalinn gateway
. *Build ReScript UI* for port configuration
. *Configure firewalld* default-deny rules
. *Test with container-hater* (your son!)
