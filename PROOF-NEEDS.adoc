...
== Current State (2026-06 update — ociLayoutEnforcement {plus} absolutePathRejection DISCHARGED)

* *LOC*: ~60,900
* *Languages*: Elixir, ReScript, Idris2, Zig, Rust
* *Postulates before*: 26 across 5 files {plus} 2 `assert_total` (spec crash stubs)
...
strengthened with `Not (component = "")` {plus} explicit `rest` index.
...
_fundamental_ String-primitive axioms added to StringLemmas — `eqStringSym`
...
leading component made every absolute path "/rest" = "" {plus}{plus} "/" {plus}{plus} rest count as
SafePath — i.e. `absolutePathRejection` was _false as stated_. The non-empty
...
verified nothing). It now builds end-to-end (`vordr.ipkg` 6/6 {plus} executable,
...
`decEqList` → element-agnostic `isNil` ({plus} `totalVulns` made public export);
...
(undefined `SecurityConfig`/`AuthorizationLevel`/`Admin`/`isPrivileged` {plus} an
...
as `partial`{plus}`idris_crash` stubs. Any caller of a `partial` function is itself
...
merely the `if` — `verifyChain` was _recursive {plus} `partial`_, so Idris2
...
total 4-case Bool lemma `boolCommTrue : a && (b && True) = b && (a && True)`. `chainHeadValid`/`chainTailValid`/`chainImpliesIndividual`/
...
[[layout-arithmetic--3-postulates--refl-srcabilayoutidr]]
...
[[contiguity--5-postulates--refl-srcabiproofsidr]]
=== Contiguity — 5 postulates → Refl (src/abi/Proofs.idr)

[cols=",,",options="header",]
|===
|Postulate |Proof |Notes
|`serviceSpecContiguous0` |`Refl` |0{plus}8=8 — fieldEnd reduces on concrete MkFieldLayout
|`serviceSpecContiguous1` |`Refl` |8{plus}8=16
|`serviceSpecContiguous2` |`Refl` |16{plus}8=24
|`serviceSpecContiguous3` |`Refl` |24{plus}8=32
|`serviceSpecContiguous4` |`Refl` |32{plus}4=36
|===

[[chain-properties--2-postulates--structural-proofs-cerro-torre-signatureproofsidr]]
...
|`chainImpliesIndividual` |Induction on `IsElem` {plus} chainHead/chainTail helpers |Replaced boolean `elem` with type-level `IsElem`
...
[[unit-return--3-postulates--trivial-theoremsidr-importerproofsidr]]
...
|`tamperEvidence` |Theorems.idr |sha256CollisionResistant {plus} signatureNonReplayable {plus} list concat non-injectivity
...
* 2026-05: via `isPrefixOfBridge` {plus} `unpackAppend` {plus} the proven
`charsPrefixOfAppend`, the three "prefix of root {plus}{plus} …" postulates
(`extractionSafety`, `symlinkSafety`, `zipSlipPrevention`) became real proofs.
* 2026-06: `ociLayoutEnforcement` (via `eqStringSym` {plus} total `anyMono`) and
`absolutePathRejection` (via `unpackEmptyInv` {plus} `unpackAppend`, plus a SafePath
...
|`extractionSafety` |ImporterProofs.idr |*PROVEN* (isPrefixOfBridge {plus} unpackAppend {plus} charsPrefixOfAppend)
|`symlinkSafety` |ImporterProofs.idr |*PROVEN* (idem)
|`zipSlipPrevention` |ImporterProofs.idr |*PROVEN* (idem)
|`normalizedIsSafe` |ImporterProofs.idr |*PROVEN 2026-06* (was false-as-stated {plus} postulate). SafePath redesigned to match normalizePath; proof = `joinBySafe ∘ mkAllSafe`. Trusted base {plus}= `splitNoDelim`, `dotDotInfixOfJoin` (2 fundamental String-primitive axioms)
|`absolutePathRejection` |ImporterProofs.idr |*PROVEN 2026-06* (SafePath redesigned: SafeSingle {plus} `charsElem '/'`-free {plus} non-empty rest; `unpackEmptyInv`/`unpackAppend`; `isPrefixOf "/" ""`/native unfold reduce by `Refl`)
|`ociLayoutEnforcement` |ImporterProofs.idr |*PROVEN 2026-06* (`anyMono` {plus} `andLeftTrue` {plus} `eqStringSym`)
|===
...
[arabic]
. [line-through]#Add `isPrefixOfBridge` {plus} `unpackAppend`# — DONE 2026-05-18.
. [line-through]#Derive extractionSafety / symlinkSafety / zipSlipPrevention# — DONE.
. [line-through]#`absolutePathRejection`# — DONE 2026-06. Strengthened `SafePath`
(`Not (component = "")` {plus} explicit `rest`), then case-split: SafeEmpty via
`isPrefixOf "/" "" = False` (Refl); SafeComponent via `slashPrefixThroughAppend`
(head char of a non-empty component is the separator), using `unpackAppend` {plus}
`unpackEmptyInv`. Note: needed a real *soundness fix* (see Current State).
. [line-through]#`ociLayoutEnforcement`# — DONE 2026-06. `elem` unfolds to `any (m ==)`;
...
. `normalizedIsSafe` (ONLY remaining DISCHARGE-PENDING): the hard one. Needs a
...
(≈ several hundred lines {plus} further bridges), deliberately deferred rather
...
[[escape-hatch-enumeration-2026-05-27--check-trusted-basesh-site-list]]
== Escape-Hatch Enumeration (2026-05-27 — `check-trusted-base.sh` site list)

Per https://github.com/hyperpolymath/standards/blob/main/docs/TRUSTED-BASE-REDUCTION-POLICY.adoc[standards++#++203],
...
* *AXIOM-STUB* — spec-stub `partial`{plus}`idris_crash` that IS the trusted-base
...
[[importerproofsidr-17-partial-sites-lines--the-partial-keyword-2026-06-normalizedissafe-discharge]]
...
|`container-stack/cerro-torre/verification/idris/ImporterProofs.idr:204` |`appendEmptyLeft` |AXIOM-TRANSITIVE |`a {plus}{plus} b = "" → a = ""`; via `unpackAppend` {plus} `unpackEmptyInv` (List-Char part total)
...
|`container-stack/cerro-torre/verification/idris/ImporterProofs.idr:290` |`mkAllSafe` |AXIOM-TRANSITIVE |per-component safety from `filter`/`split` {plus} the 2 axioms
|`container-stack/cerro-torre/verification/idris/ImporterProofs.idr:312` |`normalizedIsSafe` |*AXIOM-TRANSITIVE (PROVEN 2026-06)* |was DISCHARGE-PENDING (and false-as-stated). `joinBySafe ∘ mkAllSafe`; trusted base {plus}= `splitNoDelim`, `dotDotInfixOfJoin`
|`container-stack/cerro-torre/verification/idris/ImporterProofs.idr:333` |`extractionSafety` |AXIOM-TRANSITIVE |`isPrefixOfBridge` {plus} `unpackAppend` {plus} `charsPrefixOfAppend`
...
|`container-stack/cerro-torre/verification/idris/ImporterProofs.idr:384` |`slashPrefixAppendEq` |AXIOM-TRANSITIVE |Helper; `unpackAppend` {plus} Refl reductions
...
|`container-stack/cerro-torre/verification/idris/ImporterProofs.idr:506` |`ociLayoutEnforcement` |AXIOM-TRANSITIVE (PROVEN 2026-06) |`eqStringSym` {plus} total `anyMono`
...
[[stringlemmasidr-4-string-axioms-the-boolany-lemmas-are-all-total]]
...
|`container-stack/cerro-torre/verification/idris/StringLemmas.idr:186` |`unpackAppend` |AXIOM-STUB |`unpack` distributes over `{plus}{plus}`
...
|`container-stack/cerro-torre/verification/idris/Theorems.idr:123` |`supplyChainIntegrity` |AXIOM-TRANSITIVE |Proof = two `rewrite` {plus} `wf1` (total)
|`container-stack/cerro-torre/verification/idris/Theorems.idr:161` |`tamperEvidence` |AXIOM-STUB |Composition of `sha256CollisionResistant` {plus} `signatureNonReplayable`
