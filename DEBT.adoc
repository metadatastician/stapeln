...
link:PROOF-NEEDS.adoc[`PROOF-NEEDS.adoc`] · link:PROOF-BACKLOG.adoc[`PROOF-BACKLOG.adoc`] ·
link:TEST-NEEDS.adoc[`TEST-NEEDS.adoc`] · link:docs/proof-debt.md[`docs/proof-debt.md`] ·
...
|link:#6-cicd-debt[CI/CD] |8 (1 RESOLVED, 1 PARKED) |HIGH
|link:++#7-supply-chain--security-debt++[Supply chain / security] |2 (1 RESOLVED) |MEDIUM
...
`ROADMAP.adoc` (208 lines, substantive) and `ROADMAP.adoc` (12 lines) coexist. The `.adoc` is
...
*Next move:* repoint `docs/READINESS.adoc` at `ROADMAP.adoc`, then delete the alias.
...
=== D-6 · `ABI-FFI-README.adoc` documents functions that do not exist — MEDIUM
...
Detail lives in link:PROOF-NEEDS.adoc[`PROOF-NEEDS.adoc`] (324 lines) and
link:PROOF-BACKLOG.adoc[`PROOF-BACKLOG.adoc`] (148 lines).
...
*RESOLVED 2026-09-08* on branch `ci/drop-submodule-seam`, under owner ruling 2 of
2026-09-07: "DROP submodules; the emitted bundle {plus} generated stack declaration is the
contract". `container-stack-smoke.yml` is deleted together with the seam it claimed to check,
so the repository no longer advertises a check that cannot fail. Note for the record that its
last run on `main` (33037460362, head `8a7feec`, 2026-08-27) reported *success* over five
empty directories — that green was an artefact of `continue-on-error`, not evidence.
Each component now builds and publishes from its own repository under `metadatastician/`.

[[ci-7--svalinn-affine-buildyml-deleted-affine-port-parked-as-migrate-3--parked]]
=== CI-7 · `svalinn-affine-build.yml` deleted; the `.affine` port is parked as MIGRATE-3 — PARKED

The workflow compiled `container-stack/svalinn/src/++**++/++*++.affine` to WebAssembly. Its
subject never existed in this repository: `container-stack/svalinn` was an uninitialised
gitlink. Its last run on `main` (34196692261, head `c34db2a2`, 2026-09-08T06:53:31Z) failed
at *Build the affinescript compiler* with `error: can't open patch
'.../container-stack/svalinn/patches/affinescript-wasm-ctor-link.patch': No such file or
directory` and exit code 128. Under owner ruling 2 of 2026-09-07 the submodule seam is
dropped, so that path can never resolve here again.

The file is deleted rather than commented out or left in place: a workflow that cannot run is
not a gate, and deletion with a pointer is the honest form of "disable".

*Parked as:* **MIGRATE-3 · svalinn `.affine` policy-engine port**, post-v1. The work belongs to
`metadatastician/svalinn`, which owns the `.affine` sources; when it resumes, the build gate is
written there, against files that exist.
...
*RESOLVED 2026-09-08* on branch `ci/drop-submodule-seam`, under owner ruling 2 of
2026-09-07, by the third option: the five gitlinks, `.gitmodules` and `container-stack/` are
removed, and stapeln depends on the published artefacts. The five component repositories are
`metadatastician/++{++cerro-torre,rokur,selur,svalinn,vordr++}++` — note that every URL in the
deleted `.gitmodules` pointed at `github.com/hyperpolymath/…`, an owner that does not host
them, so the seam had never been fetchable. The contract is now the emitted bundle plus the
generated stack declaration.
...
* link:ROADMAP.adoc[`ROADMAP.adoc`] — intended direction
