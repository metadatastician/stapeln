...
[arabic]
. *Lago Grey* creates minimal, secure base images (like `lago-grey:latest`)
. *Cerro Torre* builds your application containers on top of lago-grey base
. *stapeln* orchestrates both in the stack designer
...
. *Auto-selects* lago-grey as base image (from Settings default)
. *Shows indicator*: "Base: lago-grey 2MB ✅"
. *Security score*: Automatically higher because of minimal base
. *User clicks* component to configure
. *Base Image section* shows:
* Current: lago-grey (recommended)
* Alternatives: Alpine, Chainguard, Distroless
* ++[++Customize++]++ button
. *User clicks ++[++Customize++]++*
. *Lago Grey designer opens* with toggles:
* Include shell? ❌ (recommended: no)
* Include ca-certs? ✅ (recommended: yes)
. *Real-time size update*: "2.1 MB (with your selections)"
. *Security score update*: "98/100 ✅"
. *User clicks ++[++Build Image++]++*
. *stapeln* generates lago-grey config and includes in Cerro Torre build
...
[arabic]
. *Visual Base Image Layers*
* Show each layer in the image
* Click to expand/inspect
* Visual diff between lago-grey and Alpine
. *Base Image Templates*
* "Python app base" (lago-grey {plus} python3)
* "Node.js app base" (lago-grey {plus} node)
...
[arabic]
. ✅ Component type in affinescript (`LagoGrey`)
. ✅ Related project in ECOSYSTEM.scm
. ✅ Integration point (base-image-designer)
. ✅ Visual UI space (Base Image Configuration panel)
. ✅ Security analysis integration
. ✅ GraphQL schema extension
. ✅ Elixir backend module
. ✅ miniKanren security rules
. ✅ Cerro Torre export integration
