...
|*Unit tests (Deno/TS)* |80 |tests/stapeln.test.js (16) {plus} tests/unit/container_types_test.ts (27) {plus} tests/property/nickel_config_properties_test.ts (20) {plus} tests/e2e/container_lifecycle_test.ts (7) {plus} tests/aspect/security_test.ts (10)
...
[[idris2-testsidris2--estate-port-811-2026-05-20]]
=== Idris2 (tests/idris2/) — estate port 8/11 (2026-05-20)

107 / 107 tests ported from the 5 Deno {plus} 1 JS suites above. Same assertions,
...
* [x] Unit tests — 186 Rust inline {plus} 80 Deno
...
* [x] E2E tests — full pipeline tests in both selur and vordr (17 Rust {plus} 7 Deno)
...
* [x] Aspect tests — security, concurrency, resilience in selur aspect_test.rs {plus} Deno security_test.ts
* [x] Benchmarks baselined — all 3 bench files compile with Criterion

== Remaining Gaps (for CRG B{plus})
...
|CI runner |GitHub Actions workflow for E2E {plus} property {plus} aspect suites |`.github/workflows/e2e.yml`
|===
...
|===
|Suite |Count |Status
|Unit tests (Deno/TS) |105 |All passing (80 {plus} 25 property)
