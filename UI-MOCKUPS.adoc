...
[arabic]
. *Week 1*: Build attack surface analyzer with real-time scoring
. *Week 2*: Add simulation mode with packet animation
. *Week 3*: Create "spaceship customizer" config screen
. *Week 4*: User testing with container-hater (your son!)
...
✅ User understands why something is dangerous (visual {plus} plain English)
